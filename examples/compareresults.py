        ...
        for fbn in fbionomp:
            for fbe in fbioeram:
               if fb == fd ==fbn ==fbe:
                ...
                print(f"nan_mean: {np.nanmean(loadedb)} : {np.nanmean(loadedd)}, diff = {np.nanmean(loadedd) - np.nanmean(loadedb)}")
                print(f"nan_mean: {np.nanmean(loadedbnp)} : {np.nanmean(loadedd)}, diff = {np.nanmean(loadedd) - np.nanmean(loadedbnp)}")
                print(f"nan_mean: {np.nanmean(loadedbep)} : {np.nanmean(loadedd)}, diff = {np.nanmean(loadedd) - np.nanmean(loadedbep)}")
                print(f"nan_std: {np.nanstd(loadedb)} : {np.nanstd(loadedd)}, diff = {np.nanstd(loadedd) - np.nanstd(loadedb)}")
                print(f"nan_std: {np.nanstd(loadedbnp)} : {np.nanstd(loadedd)}, diff = {np.nanstd(loadedd) - np.nanstd(loadedbnp)}")
                print(f"nan_std: {np.nanstd(loadedbep)} : {np.nanstd(loadedd)}, diff = {np.nanstd(loadedd) - np.nanstd(loadedbep)}")
